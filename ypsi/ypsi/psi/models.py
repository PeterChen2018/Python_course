...
class Shop(models.Model):
    ...
    # opendate = models.DateField(help_text="開業日期",default=datetime.now())
    opendate = models.DateTimeField('開業日期',default=timezone.now )
    #hidden = models.BooleanField(default=False,help_text="是否隱藏") 更新時增加
...
class PaySlip(models.Model):
    ...
    pay = models.DecimalField(max_digits=7,decimal_places=2,help_text="工資")
    # date = models.DateField(default=datetime.now(),help_text="發薪日期")
    date = models.DateTimeField('發薪日期',default=timezone.now )
    note = models.CharField(help_text="備註",max_length=100,blank=True)
    def __unicode__(self):
        return u"%s %s 薪資 %s" %(self.date,self.staff,self.pay)
    class Meta:
...
class Customer(models.Model):
    ...
    joindate = models.DateTimeField('加入日期',default=timezone.now )
    hidden = models.BooleanField(default=False,help_text="删除标记")
...
class Products(models.Model):
    ...
    barcode = models.CharField(max_length=20,blank=True,null=True,help_text="條形碼")
    image = models.ImageField(upload_to='products',blank=True,null=True,help_text="商品圖片")
    category = models.ForeignKey(Category,help_text="類別")
    # size = models.CommaSeparatedIntegerField(max_length=20,help_text="尺寸,請使用','號分割,如230,180",blank=True,null=True)
    size = models.CharField(max_length=20,help_text="尺寸,請使用','號分割,如230,180",blank=True,null=True)
    hidden = models.BooleanField(default=False,help_text="刪除標記")
    note = models.CharField(max_length=100,blank=True,null=True,help_text="備註")
    def _get_full_name(self):
        return ("%s|%s|%d") %(self.name,self.barcode,self.id)
    full_name = property(_get_full_name)
    def _get_avg_value(self):#獲取出庫商品平均值
    ...
    def __unicode__(self):
        return u"%s 當前均價 %s 剩餘庫存 %s" %(self.name,self.p_str[0],self.p_str[1])
    class Meta:
...
class InStream(models.Model): #出入庫分離
    code = models.CharField(max_length=30,unique=True,help_text="入庫單號")
    supplier = models.SmallIntegerField(choices=sp_choices,help_text="供應商")
    # date = models.DateField(help_text="日期",default=datetime.now())#改為Date
    date = models.DateTimeField('日期',default=timezone.now )
    keeper = models.ForeignKey(Staff,related_name='in_keeper_set',help_text="倉管")
    staff1 = models.ForeignKey(Staff,related_name='in_sells_1_set',help_text="收貨人一")
    staff2 = models.ForeignKey(Staff,related_name='in_sells_2_set',blank=True,null=True,help_text="收貨人二")
    note = models.CharField(max_length=100,blank=True,null=True,help_text="備註")
    log = models.TextField(max_length=1500,blank=True,null=True,help_text="備註")
    hidden = models.BooleanField(default=False,help_text="刪除標記")
    def get_total_amount(self):#自定義屬性
...
class InDetail(models.Model):
    inid = models.ForeignKey(InStream,help_text="入庫單ID")
    product = models.ForeignKey(Products,help_text="商品")
    value = models.DecimalField(max_digits=8,decimal_places=2,help_text="廠方進價")
    quantity = models.IntegerField(help_text="數量")
    depot = models.ForeignKey(Depot,help_text="出入庫庫房")
    depotdetail = models.CharField(max_length=100,blank=True,null=True,help_text="出入庫詳細位置")
    def _get_amount(self): #自定義屬性金額小計
        return self.quantity*self.value
    amount = property(_get_amount)
    def __unicode__(self):
        return (u"%d:%s 單價%d 共%d 套，小計%s 元") %(self.inid_id,self.product.name,self.value,self.quantity,self.amount)#此處使用了%s而非%d提高Decimal類型數據精度
    class Meta:
...
class OutStream(models.Model):
    ...
    date = models.DateTimeField(help_text="日期",default=timezone.now )
...
class SellOrderDetail(models.Model):
    oid = models.ForeignKey(SellOrder,help_text="銷售單據號")
    product = models.ForeignKey(Products,help_text="商品")
    quantity = models.IntegerField(help_text="數量")
    price = models.DecimalField(max_digits=8,decimal_places=2,help_text="價格")
    def _get_amount(self): #小計金額
        return self.quantity*self.price
    amount = property(_get_amount)
    def __unicode__(self):
        return u'訂單%d: %d 件 %s，共 %s 元' %(self.oid.id,self.quantity,self.product.name,self.amount)
    class Meta:
        verbose_name_plural = ('銷售詳單')


class Remit(models.Model):#增加匯款記錄管理
    supplier = models.SmallIntegerField(choices=sp_choices,help_text="供應商")
    amount = models.DecimalField(max_digits=8,decimal_places=2,help_text="金额")
    staff = models.ForeignKey(Staff,help_text="匯款人")
    # date = models.DateField(default=datetime.now(),help_text="匯款日期") #修改為日期
    date = models.DateTimeField('匯款日期',default=timezone.now )
    hidden = models.BooleanField(default=False,help_text="刪除標記")
    note = models.CharField(max_length=100,blank=True,null=True,help_text="備註")
    def __unicode__(self):
        return u"%s %s 匯出 %s 元" %(self.date,self.staff,self.amount)
    class Meta:
