...
def ypsi_sell_list(ds):
    if isinstance(ds,int):
        ...
        # return date_list,simplejson.dumps(s_str,ensure_ascii=False) # 转 JSON 数据避免中文解码问题
        return date_list,json.dumps(s_str,ensure_ascii=False) # 转 JSON 数据避免中文解码问题
...
def user_login(request):
    ...
    if act == "check":
        ...
        if act == "out":
            print 'out'
            page_title = "用戶名稱已經註銷，請重新登錄"
        elif act == "err":
            print 'err'
            page_title="用戶名稱錯誤或密碼錯誤"
        form = YLogin()
        request.session["next_path"] = request.GET.get("next","")
    return render_to_response('accounts/login.html',locals())
...
@login_required
def ypsi_index(request):
    request.session.set_expiry(0) # 關閉瀏覽器 session
    ...
    try:
       user = User.objects.get(username=name)
       userinfo = Staff.objects.get(user=user)
    ...
    wmode = request.GET.get("w","")
    page_errs=""
    # date_list(12) # 讀取指定時間段
    days = request.GET.get('days',7)
    try:
        days = int(days)
    except :
        page_errs=("請輸入正確參數")
    if level > 5 and wmode <> "1" and days > 60 :
        page_errs=("普通模式下可統計天數上限為60日 <a href='?w=1&days=120'><span><img src='/static/images/css/chart16.png'/> 切換寬螢幕模式可以看更多</span></a> 或 <a href='javascript:history.go(-1)'>返回上一頁</a>")
    elif level > 5 and wmode == "1" and days > 120 :
        page_errs=("店長以下人員,可統計天數上限為120日")
    if page_errs == "":
        dlist = ypsi_sell_list(days)[0]
        s_str=ypsi_sell_list(days)[1]
        page_title = u"首頁"
        ...
        if act == "getData":
            ...
            for s in slist:
                jStr["slist"].append({"sid":s.id,"shop":s.shop.name,"staff":s.staff.name,"amount":s.total,"date":datetime.datetime.strftime(s.date,'%Y-%m-%d %H:%M:%S')})
            # return HttpResponse(simplejson.dumps({"page_title":page_title,"plist":jStr["plist"],"slist":jStr["slist"],"dlist":dlist,"s_str":s_str},ensure_ascii=False), mimetype="text/plain")
            return HttpResponse( json.dumps({"page_title":page_title,"plist":jStr["plist"],"slist":jStr["slist"],"dlist":dlist,"s_str":s_str},ensure_ascii=False), mimetype="text/plain")
        else:
            return render_to_response("app/index.html",{"page_title":page_title,"err":page_errs,"user":user,"last_login":last_login,"plist":plist,"slist":slist,"dlist":dlist,"s_str":s_str,"wmode":wmode})
    else:
        return HttpResponse("<html><center>%s</center></html>"%page_errs)
...
def ypsi_depots_in(request):
    ...
    if act == "add":
        ...
        if request.POST:
            ...
            if form.is_valid():
                fc = form.cleaned_data
                # save to Instream table
                InStream(code=fc["code"],supplier=fc["supplier"],date=fc["date"],keeper=fc["keeper"],staff1=fc["staff1"],note=fc["note"]).save()
                return HttpResponseRedirect("?act=added")

        else:
            form = yforms.InStream()
            print 'yforms.InStream()'
        return render_to_response('app/depots_instream.html',{"page_title":page_title,"form":form,"act":act})
...
def ypsi_sales(request):
    ...
    return render_to_response('app/sales.html',{"page_title":page_title,"level":level})
...
def ypsi_sell_list(ds):
    if isinstance(ds,int):
        ...
        if len(SellOrder.objects.all())>0:#避免空数据错误
            ...
            cursor.close()
            del shop_list,cursor,today
        return date_list, json.dumps(s_str,ensure_ascii=False) #转JSON数据避免中文解码问题
